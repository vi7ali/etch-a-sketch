...
function changeColor(e) {
	...
	if(numberOfPasses>0 && !randomColorOption.checked) {
		let darkerColor = currentColor.slice(4, currentColor.length-1);
		darkerColor = darkerColor.split(",");

		for(i=0;i<darkerColor.length;i++) {
			let originalPercent = ((darkerColor[i] * 100)/(100 - numberOfPasses * 10))*0.1; // 10% of the original value using reverse percentage formula
			darkerColor[i] = darkerColor[i] - originalPercent;
		}
		cell.style.backgroundColor = `rgb(${darkerColor.join()})`;
	}
	else cell.style.backgroundColor = `rgb(${color[0]}, ${color[1]}, ${color[2]})`;	

	if(!randomColorOption.checked && numberOfPasses<10)	{
		numberOfPasses++;
		cell.setAttribute("data-passes", numberOfPasses);
	}
	
	if(randomColorOption.checked) cell.setAttribute("data-passes", 1);	
}
